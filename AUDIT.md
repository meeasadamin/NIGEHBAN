...
| N6 | **Published (2026-09-14):** public repository [meeasadamin/NIGEHBAN](https://github.com/meeasadamin/NIGEHBAN) with full history. Added community health files (CONTRIBUTING, SECURITY, CODE_OF_CONDUCT, CITATION.cff, CHANGELOG), issue forms, a PR template, Dependabot for Actions, and SHA-pinned CI actions. Enabled vulnerability alerts, Dependabot security updates, private vulnerability reporting, and secret scanning with push protection. Streamlit deployment is still not done, so no live link is claimed | GitHub API responses (201 create; 200/204 settings) | CI badge on README; Actions tab |
| N7 | **Deployed:** live app at [nigehban.streamlit.app](https://nigehban.streamlit.app/), deployed by the owner from `main`. Checked before linking: loaded in Edge, 3 KPI gauges and the NIGEHBAN header rendered, no error alerts. **Defect found on the live app only:** Streamlit Cloud's Fork / GitHub buttons overlapped the top-right of the brand header, because the header had been moved up in V13. **Fix:** extra top padding only when those toolbar buttons exist; locally the header still starts 16px from the top | Edge screenshot of the live app | README live-demo line and badge; repository website field |
...
| V1 | **Change (user request):** sidebar logo moved into Streamlit's native header slot (`st.logo`, rendered by `scripts/build_logo.py`), level with the collapse button and shown as an icon when the sidebar is collapsed | Edge at 1440px and 390px | `tests/test_app_smoke.py` (app renders); visual check |
| V2 | **Change (user request):** KPI meter bars replaced with SVG speedometer gauges: value arc in the level colour, red High-zone band starting at the hazard's threshold, threshold tick, needle, printed percentage. Animation respects `prefers-reduced-motion` | Gauge geometry unit-tested | `tests/test_accessibility.py::test_gauge_draws_value_and_high_zone_from_the_threshold`; `scripts/check_layout.py` (28px/700 level word, no overflow) |
| V3 | **Defect found in this pass:** the calibrated-probability table drew the High bar in blue (`primaryColor`, the Low colour). **Fix:** tables rebuilt as styled HTML; each bar wears its own row's level colour with shape and word printed | Before/after Edge screenshots | `tests/test_accessibility.py::test_tables_escape_plain_text_and_keep_trusted_markup`; 27 new contrast pairs (54 total) |
| V4 | **Defect found in this pass:** the hotspot table hid its last column and the stress-test and performance tables overflowed at 1440px. **Fix:** district and province merged into one two-line cell, shorter headers with a note explaining the arrows; headline numbers moved into stat tiles | Edge at 1440px: all columns visible; 390px: tables scroll inside their own region | `scripts/check_layout.py` |

| V5 | **Change (user request):** sidebar card headings redesigned: brand-green icon tile, 16px bold title, one-line hint | Edge at 1440px | Visual check; icon-tile and title contrast pairs in `tests/test_accessibility.py` |
| V6 | **Change (user request):** download controls moved into a titled, green-accented bar with a solid "Download CSV" and an outlined "Download report" button | Edge at 1440px and 390px (buttons stack full width) | `tests/test_app_smoke.py::test_three_kpi_cards_charts_and_two_downloads`; button text contrast (normal and hover) |
| V7 | **Enhancement (user request):** district profile now shows "What stands out" tiles (the three inputs furthest from the national middle), grouped rows (simulated vs real geography), changed rows highlighted with the original dataset value, a relative "vs province" chip, and a range bar per input (national min to max, scenario dot, province-median tick) with a plain-language rank. The engine returns the raw numbers behind these | Edge at 1440px | `tests/test_risk_engine.py::test_district_profile_marks_changes_and_sources` (range position, vs-province %), `tests/test_accessibility.py::test_profile_parts_state_meaning_in_text_not_only_graphics` |
| V8 | **Change (user request):** model card split into three sub-cards with icon headings and status badges: Provenance checks ("4 of 4 passed"), Performance (decision rule as a callout), Limitations (amber card, numbered items) | Edge at 1440px and 390px | `tests/test_app_smoke.py::test_model_card_checklist_shows_every_check_passing`, `tests/test_accessibility.py::test_model_card_headings_have_icons_and_escaped_badges` |

| V9 | **Change (user request):** design system pass. Each section has its own accent (green, teal, violet, deep gold; none is a risk-level colour) on the number tile, eyebrow, tabs, hazard picker and panel top rules. A gold rule taken from the emblem's eye runs under every section heading. The page sits on a soft tint with one level of white panels (the grey section boxes were removed). One spacing scale (`SPACE` in `ui_theme.py`) sets every gap and padding | Edge at 1440px and 390px | 22 new contrast pairs (93 total), `tests/test_accessibility.py::test_every_section_accent_is_distinct_from_the_level_palette` |
| V10 | **Defect found in this pass:** Streamlit gives every markdown block `margin-bottom: -1rem`, which cancelled the layout gap, so the banner touched the header and headings touched the data below them. **Fix:** the pull is removed for blocks without a trailing paragraph | Edge DOM check: header-to-banner gap 0px before, 16px after | `scripts/check_layout.py`; visual check |
| V11 | **Change (user request):** headings rewritten as questions with a coloured eyebrow and an icon tile (e.g. "What drives the flood score", "Where do hazards pile up?", "Can you trust these scores?") | — | `tests/test_app_smoke.py::test_hazard_selector_scopes_explanation_and_map` |
| V12 | **Change (user request):** district profile reorganised into three groups of input cards matching the sidebar (Climate, Land and exposure, Geography). Each card shows the value, a national range bar, a short rank ("Highest 4% in Pakistan"), the difference from the province and a ★ flag when the value is more extreme than 90% of districts. The full table stays available in an expander | Edge at 1440px: four columns, no lone cards | `tests/test_app_smoke.py::test_district_profile_groups_every_input_into_cards`, `tests/test_accessibility.py::test_profile_card_flags_standouts_and_changes_in_words` |
| V13 | **Change (user request):** brand header moved up. Streamlit's top bar is made transparent on wide screens, so the header starts 16px from the top (was about 68px). Phones keep the offset because the sidebar toggle lives in that bar | Edge: brand top 16px at 1440px | `scripts/check_layout.py` (header not clipped) |
